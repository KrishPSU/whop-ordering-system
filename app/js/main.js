...
function renderMenu(data) {
  ...
  // Move 'Pizza Toppings' category to the front if it exists
  const pizzaToppingsIndex = data.findIndex(cat => cat.category === 'Pizza Toppings');
  if (pizzaToppingsIndex > 0) {
    const [pizzaToppings] = data.splice(pizzaToppingsIndex, 1);
    data.unshift(pizzaToppings);
  }
  ...
  data.forEach(category => {
    ...
    category.items.forEach(item => {
      if (item.small || item.large || item.default) {
    ...
    itemsWithPricing.forEach(item => {
      ...
      if (item.small && item.large) {
        priceText = `Small: ${item.small} | Large: ${item.large}`;
      } else if (item.small) {
        priceText = `Small: ${item.small}`;
      } else if (item.large) {
        priceText = `Large: ${item.large}`;
      } else if (item.default) {
        priceText = `${item.default}`;
      }
